...
Finally, since we want to make the exchange rule admissible, we have to build part of a permutation into the rules as well.
This leads us to the theory shown in \cref{fig:props}.

[TODO: Ugh, want to use subscripts to list a bunch of generators, but also to label the multiple outputs of each generator.]

\begin{figure}
  \centering
  \begin{mathpar}
    \inferrule{}{\vec x:\vec A\types (\sigma(\vec x,{\vec f}_1^{\fa_1},\dots,{\vec f}_n^{\fa_n})\mid g_1^{\vec{\fb}}):\actv {\vec B}}
  \end{mathpar}
