...
\begin{figure}
  \centering
  \begin{mathpar}
    \inferrule{ }{\Gamma,x:A \types x\pc A}\and
    \inferrule{
      \Gamma \types M_1\pc A_1 \\ \Gamma\types M_2\pc A_2 \\ \cdots \\ \Gamma\types M_n\pc A_n \\\\
      f\in \cG(A_1,\dots,A_n; B_1,\dots,B_m)\\ 1\le j\le m
    }{
      \Gamma \types f_j(M_1,\dots,M_n)\pc B_j
    }\and
    \inferrule{
      \Gamma \types M_1\pc A_1 \\ \Gamma\types M_2\pc A_2 \\ \cdots \\ \Gamma\types M_n\pc A_n \\\\
      f\in \cG(A_1,\dots,A_n; \cdot )
    }{
      \Gamma \types f_j(M_1,\dots,M_n)\pc ()
    }\and
    \inferrule{\Gamma\types M\pc A \\ \Gamma\types N\pc B }{ \Gamma\types \pair{M}{N} \pc A\tensor B }\and
    \inferrule{ }{\Gamma\types \ttt\pc\unit}\and
    \inferrule{\Gamma\types M\pc A\tensor B}{\Gamma\types \pi_1(M) \pc A}\and
    \inferrule{\Gamma\types M\pc A\tensor B}{\Gamma\types \pi_2(M) \pc B}\and
    \inferrule{\Gamma\types M\pc \unit}{\Gamma\types \cancel{M}:()}\and
  \end{mathpar}
...
\begin{figure}
  ...
  \begin{mathpar}
    \inferrule{
      x_1:A_1,\dots,x_n:A_n\types (M_1,\dots,M_m\mid Z_1,\dots,Z_p):(B_1,\dots,B_m)\\
      \sigma\in \Sigma_n \\ \tau\in\Sigma_m \\ \rho\in\Sigma_p
    }{
      x_{\sigma 1}:A_{\sigma 1},\dots,x_{\sigma n}:A_{\sigma n}\types (M_{\tau 1},\dots,M_{\tau m}\mid Z_{\rho 1},\dots,Z_{\rho p}):(B_{\tau 1},\dots,B_{\tau m})
    }\and
    \inferrule{ }{() \types ():()}\and
    \inferrule{\Gamma\types (\vec{M}\mid \vec{Z}):\Delta\\ \types A\type}{\Gamma,x:A\types (\vec{M},x\mid \vec{Z}):(\Delta,A)}\and
    \inferrule{
      \Gamma\types (\vec M,\vec N\mid\vec{Z}):(\Xi,\Psi) \\
      f\in\cG(\Psi,\Phi; \Delta)\\
      |\Delta|>0
    }{
      \Gamma,\vec{x}:\Phi \types (\vec M,\vec f(\vec N,\vec x)\mid\vec{Z}):(\Xi,\Delta)
    }\and
    \inferrule{
      \Gamma\types (\vec M,\vec N\mid\vec{Z}):(\Xi,\Psi) \\
      f\in\cG(\Psi,\Phi; \cdot)
    }{
      \Gamma,\vec{x}:\Phi \types (\vec M\mid\vec{Z},\vec f(\vec N,\vec x)):\Xi
    }\and
    \inferrule{\Gamma\types (\vec{M},N,P\mid\vec{Z}):(\Delta,A,B)}{\Gamma\types (\vec{M},\pair N P\mid\vec{Z}):(\Delta,A\tensor B)}\;\tensorI\and
    \inferrule{\Gamma\types (\vec{M},P\mid\vec{Z}):(\Delta,A\tensor B)}{\Gamma\types (\vec{M},\pi_1(P),\pi_2(P)\mid\vec{Z}):(\Delta,A,B)}\;\tensorE\\
    \inferrule{\Gamma\types(\vec M\mid\vec Z):\Delta}{\Gamma\types (\vec M,\ttt\mid\vec Z):(\Delta,\one)}\;\one I\and
    \inferrule{\Gamma\types(\vec M,N\mid\vec Z):(\Delta,\one)}{\Gamma\types(\vec M\mid \cancel{N},\vec Z):\Delta}\;\one E\and
  \end{mathpar}
  \caption{Rules for terms}
  \label{fig:cl-smc-terms}
\end{figure}

With these notations, the term judgment is described in \cref{fig:cl-smc-terms}.
For clarity, we have made the exchange rule explicit at the beginning.
Note that it mirrors exactly the natural deduction from \cref{fig:clnatded-smc}, with the additional annotation by pre-terms.
For instance, when we annotate the two problematic derivations we obtain the same pre-term.
\begin{mathpar}
  \inferrule*[Right=$g$]{
    \inferrule*{
      \inferrule*[Right=$f$]{
...
\begin{mathpar}
  ...
  \pair{\pi_1(P)}{\pi_2(P)} \equiv P\and
  \cancel{\ttt} \equiv ()\and
\end{mathpar}
and the following relations on lists:
\begin{mathpar}
  (\vec M,\ttt \mid \vec Z,\cancel{W}) \equiv (\vec M,W\mid \vec Z)\and
...
\end{mathpar}
The conclusion of this rule is (modulo exchange) an instance of a conclusion of the $f$-generator rule.
Since the $\pi$-pre-terms introduced by the actual rule $\tensorE$ that leads to this conclusion are disjoint from the $f$-pre-terms that would be introduced by this $f$-generator rule.
