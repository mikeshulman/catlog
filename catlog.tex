...
[TODO: I like this, but it seems unlikely to really be appreciated by the reader at this point who doesn't have any exposure to type theory yet.  Maybe it should be an appendix that we refer the reader to at various points?]
...
(From a constructive or type-theoretic point of view, $W\sig$ and its initiality are sometimes a fundamental axiom not reducible to sets.)
...
Wait for \cref{chap:unary}.)

To deal with this situation, we augment our signatures with a set $\sig_0$ of \textbf{sorts} together with, for each operation $m\in\sig_1$, an \textbf{output sort} $c_m\in\sig_0$ and also a list of \textbf{input sorts} $d_{m,1},\dots,d_{m,\ay(m)}$.
For brevity we write such an operation as $m:(d_{m,1},\dots,d_{m,\ay(m)}) \to c_m$.
From now on we call these simply \textbf{signatures}; the simpler signatures of \cref{sec:trees} we re-christen \textbf{one-sorted signatures}.
...
Similarly, we define a $\sig$-\textbf{labeled tree} as before, with the additional requirement that if $x$ is the $k^{\mathrm{th}}$ child of $y$, and $x$ is labeled by $m$ while $y$ is labeled by $p$, then $c_m = d_{p,k}$.
...
Note that the signatures $\sig^{\equivsym}$ from \cref{sec:axioms} also have sorts defined like this.

Each sort in one of the signatures $\sig^{(k)}$ is called a \textbf{judgment}.
We write $\cJ$ for a generic judgment, but we use more specific and congenial notation in particular cases.
For instance, the sort $(\cJ,x,y)$ in $\sig^{\equivsym}$ is usually written $x\equiv y : \cJ$.

Each operation $m:(\cJ_1,\dots,\cJ_n) \to \cJ'$ in one of these signatures is called a \textbf{rule} and written
...
The whole sequence of signatures is called a \textbf{deductive system}.
...
Technically, something must be said about \emph{how} a linear syntax gives rise to a derivation tree.
...
If we are given a putative term claiming to represent a derivation of some judgment, then there is a further step after parsing of verifying that the parse tree indeed corresponds to a valid derivation tree; this is called \textbf{type-checking}.
The main point to be made here is that technically, a term must contain enough information to reconstruct the derivation tree; but in many cases we tend to omit some of that information when it can be inferred from context.
(Programmers even have elaborate and precise algorithms to make sense of ``inferring from context''.)

With all that said, there are exceptions: some type theories deviate from this general picture by having terms that do not exactly represent derivations.
Dependent type theories are probably the most notable example.
But even in those cases it is helpful to understand the ``standard'' picture in order to appreciate the deviancy better.
