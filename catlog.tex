...
\begin{rmk}
  ...
  So in that sense \cref{thm:category-tad} is actually saying \emph{less} than one might think; the actual work is being done by the silent step of parsing.
...
However, the introduction of abstract variables appears to break the converse property (that derivations determine unique terms).
...
% ; we will return to the tricks in \cref{cha:canonical}.
...
To start with, this term is parsed as $\acase AB(M,(u.P),(v.Q))$: that is, the periods bind more tightly than the commas.
...
These variables $u$ and $v$ are ``bound'', like the integration variable in a definite integral: $\acase AB(M,u.f(u),v.g(v))$ has the same meaning as $\acase AB(M,w.f(w),z.g(z))$ (without needing to change any variables anywhere else).
...
[TODO: Probably good to include the detail here.]
...
In keeping with the usual ``biased'' definition of monoidal category, we will say that a multicategory is \textbf{representable} if it is equipped with a chosen unit object and a chosen binary tensor product for every pair of objects.
...
However, we will need notions of limit and colimit in multicategories, and since these are less well-known we review them here.
...
By contrast, it is traditional to formulate the cut rule itself in terms of the $\circ_i$ compositions:
...
(Note, though, that to recover the $\circ_i$-style cut from the multi-cut requires composing with identities, hence invoking \cref{thm:monpos-identity} as well.)
...
For substitution, we may again choose between the $\circ_i$-style and the multi-cut.
...
The annotations $A\tensor B$ and $\Gamma|\Delta$ are to make type-checking possible (see \cref{thm:moncat-tad}); but generally we will omit them and write simply $\match_\tensor$.
...
The case of $\one$ is similar to that of $\tensor$ but simpler: to do something with an element of $\one$, it suffices to assume that it is $\ott$.
...
The qualification ``(free)'' in \cref{thm:moncat-linear} is because bound variables can have any name, so it is technically possible for a bound variable to have the same name as a variable occurring free elsewhere (though not inside the ``scope'' of the bound variable).
For instance, we could have
\[ x:A\tensor B, u:C \types f(\match_\tensor(x,uv.g(u,v)),u):D \]
However, this is arguably bad style, so we will generally assume that it is not done (we can always apply $\alpha$-equivalence to avoid it).
...
(This didn't arise in \cref{sec:catcoprod} because there it was not possible to substitute into the subterms $u.P$ and $v.Q$ of a $\case$ term containing bound variables, since they could not contain any \emph{other} variables to be substituted for.)

When talking about syntax of terms explicitly, one generally says that this is because otherwise, a $u$ or $v$ in $M$ would after substitution be ``in the scope'' of the binding of $u$ or $v$, whereas all the variables of $M$ ought to be free in the substituted term.
But when we regard substitution as an operation on derivations, the point is just that to eliminate a cut after $\tensorE$ of the following sort:
\begin{mathpar}
  \inferrule*[Right=cut]{
    \Gamma\types M:A \\
...
Once again, the ``variable management'' is fairly automatic if we consider derivations as basic, with term annotations applied afterwards modulo $\alpha$-equivalence.
...
The superscript $\Gamma,\Delta$ on $\abort$, however, is there for a different purpose, which is the same purpose as the passing of all the variables in the context as arguments to $\ttt$.
...
This is similar to how, as we have noted, cut is usually stated in type theory in $\circ_i$-style rather than as a multi-composition.
...
While we are stuck with these labels, it is probably best (for a classically trained category theorist first encountering the notion) not to read too much into them; the point is simply that we make our mathematics more general by generalizing our logic.
...
\end{tabular}
\end{center}
To be precise, all of these logics should additionally be qualified as ``intuitionistic'', since there are also ``classical'' versions of linear, affine, and relevance logics.
...
We return to the cartesian case, and since $\tensor$ and $\one$ coincide here with $\meet$ and $\top$ we omit the former.
...
\begin{ex}\label{ex:cocartesian-typetheory}
  Is there a well-behaved type theory (having admissible cut and an initiality theorem) corresponding to the ``cocartesian multicategories'' of \cref{ex:cocartesian-clubs}?
...
[TODO: Relative to natural deduction, sequent calculus is more like direct category-theoretic construction by directly applying universal properties on both sides, rather than talking about ``elements'' on the right only.
But sequent calculus still enjoys cut-elimination, so it's sort of a halfway point between diagrammatic category theory and ``fully internal'' natural deduction-style theories where objects ``contain terms''.
We could make that more explicit by introducing sequent calculus terms that mirror universal properties, like pairing and copairing as well as post- and pre-composition.
That would be interesting to do, perhaps much earlier on; should we?.]

[There are also ``Hilbert systems'', about which the less said the better.]
