...
\author{}
\date{\today}
\autodefs{\bSet\bPoset\bRelGr\bCat\bGr\bmSLat\ftype\bPrCat}
\let\types\vdash
\let\sect\section
...
In type-theoretic lingo, \cref{thm:categories-cutadm} says that \textbf{the cut rule is admissible} in the cut-free type theory of categories under \cG.
...
Thus, \cref{thm:categories-cutadm} can be rephrased to say that \textbf{substitution is admissible} in the cut-free type theory of categories under \cG.
...
% TODO: Can we have an exercise on fibrations whose fibers are meet-semilattices, using the sequent calculus to prove that $x^*$ preserves meets?
...
It turns out that for natural deduction, we can implement the necessary equivalence relation using \emph{reduction rules} that allow us to \emph{simply} more complicated derivations into equivalent simpler ones.
...
\end{equation}
Note that in these equalities, we want to allow $M$ and $N$ to be arbitrary terms.
There is another, dual, family of simplifications as well; we want to identify the following derivations of $A\times B\to A\times B$:
\begin{mathpar}
  \inferrule*{
...
For the moment we postpone consideration of these equalities as ``directed simplifications'' and simply regard them as generating an equivalence relation on terms, which we also require to be a congruence for everything else.
We can describe this ``generation'' using an additional judgment ``$x:X\types M\equiv N :A$'', with rules shown in  \cref{fig:catprod-equality}: in addition to the $\beta$ and $\eta$ reductions, we assert reflexivity, symmetry, and transitivity, and also that all the previous rules preserve equality (this ensures by induction that it is a congruence for substitution).
...
\begin{thm}\label{thm:catprod-initial}
  For any directed graph \cG, the free category-with-products $F_\bPrCat \cG$ it generates is described by the unary type theory for categories with products under \cG: its objects are the $A$ such that $\types A\type$ is derivable, and its morphisms $A\to B$ are the terms $M$ such that $x:A \types M:B$ is derivable, modulo the equivalence relation $\equiv$.
\end{thm}
\begin{proof}
  \cref{thm:catprod-subadm} tells us how to can compose morphisms in our putative $F_\bPrCat \cG$, and \cref{thm:catprod-subcong} tells us that this operation respects the equivalence relation, so it is well-defined.
  To prove that we have a category, we have to check that composition is associative and unital.
  [TODO\dots]
\end{proof}

Finally, we return to the ``directedness'' of the $\beta$ and $\eta$ reductions.

[TODO: I'm less confident about what to say here.
I suppose if we want to be precise, we have to talk about Church-Rosser and confluence properties?
Also, does it work in this simple case to consider $\eta$ as a reduction, or do we already need to make it an expansion?]
