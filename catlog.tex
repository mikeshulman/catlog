...
Before beginning the study of any type theories in particular, we spend a brief chapter on the formal apparatus underlying them.
This is rarely explained in detail, yet its unfamiliarity to mathematicians is probably a large part of what makes type theory hard for them to understand.
...
Now categorical structures are particular kinds of \emph{algebraic} structures, so it seems reasonable to think first about what free algebraic structures look like in general.
A convenient way to present these is by using \emph{trees}.

A \textbf{tree} is a set of \textbf{nodes} together with a binary relation between them called \textbf{edge existence} (a ``relational graph'') that is connected and loop-free.
A tree is \textbf{rooted} if it is equipped with a chosen node called the \textbf{root}.
In a rooted tree every node $x$ has a unique (non-retracing) path to the root; if $x$ is not the root, this path goes through a unique edge connected to $x$ that we call \textbf{outgoing}, and the node at the other end of that edge is its \textbf{parent}.
...
A node $x$ together with all its descendants forms another rooted tree with $x$ as the root, called the \textbf{branch} of $x$.
...
A rooted tree is \textbf{well-founded} if there are no infinite non-retracing paths.

Let $\cO$ be a set, whose elements we call \textbf{operations}, and $\{\cA_m\}_{m\in \cO}$ an \cO-indexed family of sets that we call the \textbf{arities}.
We call $(\cO,\cA)$ a \textbf{signature}.
A $(\cO,\cA)$-\textbf{algebra} is a set $X$ together with for every $m\in\cO$ a function $\act m : X^{\cA_m}\to X$.
There is an obvious notion of $(\cO,\cA)$-algebra \textbf{morphism}.

$(\cO,\cA)$-algebras are a very ``primordial'' sort of algebraic structure, with arbitrary operations but no axioms allowed.
For instance, if $\cO=\{e,m\}$ with $\cA_e = \emptyset$ and $\cA_m=\mathbf{2}$, then $(\cO,\cA)$-algebras are \emph{pointed magmas}: sets equipped with a basepoint and a binary operation.
Later on we will see how to add axioms.

A $(\cO,\cA)$-\textbf{labeled tree} is a rooted tree equipped with a \textbf{labeling} function from nodes to \cO, along with for every node $x$ labeled by $m\in\cO$ a bijection between the incoming edges of $x$ and $\cA_m$.
There is an obvious notion of \emph{isomorphism} between labeled trees.
We write $W(\cO,\cA)$ for the set of all isomorphism classes of \emph{well-founded} $(\cO,\cA)$-labeled trees.
(Note that $W(\cO,\cA)$ is empty unless some of the arity sets $\cA_m$ are empty.)
Then $W(\cO,\cA)$ has a $(\cO,\cA)$-algebra defined as follows: given $m\in\cO$ and $f:\cA_m\to W(\cO,\cA)$, define a tree $\act m (f)$ with nodes $\{\star\} \sqcup \bigsqcup_{a\in\cA_m} f(a)$, where $\star$ is the root, with label $m$, and its children are the roots of the trees $f(a)$, with edges labeled tautologically.

We will show that $W(\cO,\cA)$ is the initial $(\cO,\cA)$-algebra.

\begin{thm}\label{thm:tree-ind}
  Suppose $P\subseteq W(\cO,\cA)$ has the property that whenever a function $f:\cA_m\to W(\cO,\cA)$ satisfies $f(a)\in P$ for all $a$, then $\act m(f)\in P$.
  Then $P= W(\cO,\cA)$.
\end{thm}
\begin{proof}
  Suppose not, so there is a well-founded $(\cO,\cA)$-labeled tree not in $P$.
  Let $m$ be the label of its root and $f:\cA_m\to W(\cO,\cA)$ the function sending each $a$ to the branch of the child of the root labeled by $a$; then our given tree is (isomorphic to) $\act m(f)$.
  By the contrapositive of our assumption, therefore, there must be some $a$ such that $f(a)\notin P$.
  Iterating, we obtain an infinite non-retracing path, contradicting well-foundedness.
\end{proof}

\begin{thm}\label{thm:tree-rec}
  For any $(\cO,\cA)$-algebra $X$, there is a unique $(\cO,\cA)$-algebra morphism $W(\cO,\cA) \to X$.
\end{thm}
\begin{proof}
  TODO: standard argument.
\end{proof}

Note that \emph{free} $(\cO,\cA)$-algebras can be constructed by a simple modification.
Given $(\cO,\cA)$ and any set $X$, let $\cO[X] = \cO \sqcup X$, and define $\cA[X]$ to extend $\cA$ by giving each $x\in X$ arity zero.
Then an $(\cO[X],\cA[X])$-algebra is just an $(\cO,\cA)$-algebra together with a map from $X$ into its underlying set.
The initial such algebra is then the free $(\cO,\cA)$-algebra on $X$.
Thus, the forgetful functor from $(\cO,\cA)$-algebras to sets has a left adjoint.

A different way to express \cref{thm:tree-rec} is that \emph{given an arbitrary set $X$, to define a function $W(\cO,\cA) \to X$ it suffices to define an $(\cO,\cA)$-algebra structure on $X$}.
This may seem like a trivial reformulation, but it better reflects the way we will use it.


\section{Indexed trees}
\label{sec:indexed-trees}

Sometimes an algebraic structure has more than one underlying set (or ``sort'').
As a simple example, we could consider a set together with a group acting on that set to be a single algebraic structure.
(Categories could be regarded as having two sets, namely objects and arrows, but it is generally better to treat them differently; wait for \cref{chap:unary}.)

If $\cI$ is any set, an $\cI$-\textbf{indexed signature} is a signature $(\cO,\cA)$ together with functions $c:\cO\to\cI$ and $d_m:\cA_m\to \cI$.
The idea is that $c$ specifies the output sort of $m$, while $d_m$ specifies the sorts of its inputs.
If $\cA_m$ is finite, we may informally write $m : (d_m(a_1),\dots,d_m(a_n)) \to c(m)$ to indicate the input and output arities.
Thus, we define an $\cI$-\textbf{indexed algebra} for such a signature to be an \cI-indexed family of sets $X_i$ together with operations
\[ \act m:\setof{ f :{\cA_m} \to \bigsqcup_i X_i | \forall a, f(a) \in X_{d(a)} } \to X_{c(m)}\]
For instance, if $\cI=\{g,s\}$ and $\cO=\{m,t\}$ with $\cA_m=\cA_t =\mathbf{2}$, and
\begin{mathpar}
  m : (g,g) \to g \and t : (g,s) \to s
  % c(m)=g \and c(t) = s \and
  % d_m(x)=g \and d_m(y)=g\and
  % d_t(x)=g \and d_t(y)=s
\end{mathpar}
then an indexed algebra consists of two sets $X_g$ and $X_s$, a binary operation on $X_g$, and an action of $X_g$ on $X_s$.

An $\cI$-\textbf{indexed tree} is one with a function from its nodes to \cI.
Let $W_\cI(\cO,\cA)_i$ be the set of well-founded $(\cO,\cA)$-labeled and $\cI$-indexed trees such that the root is labeled by $i$, the index of each node matches the $c$-index of its \cO-label, and the index of each child of a node matches the $d$-index of the \cA-label on the connecting edge.
Then we have analogous theorems with analogous proofs:

\begin{thm}\label{thm:tree-ind}
  Suppose $P_i\subseteq W_\cI(\cO,\cA)_i$ is a family of subsets such that whenever a function $f:\cA_m\to \bigsqcup_i W_\cI(\cO,\cA)_i$ satisfies $f(a)\in P_{d(a)}$ for all $a$, then $\act m(f)\in P_{c(a)}$.
  Then $P_i= W_\cI(\cO,\cA)_i$ for all $i$.\qed
\end{thm}

\begin{thm}\label{thm:tree-rec}
  For any \cI-indexed $(\cO,\cA)$-algebra $X$, there is a unique \cI-indexed $(\cO,\cA)$-algebra morphism $W_\cI(\cO,\cA) \to X$.\qed
\end{thm}
...
Making this completely precise in general is rather technical, so we will only do [or ``begin with''?] an example.
...
Let $\cI = \F\bMag X\times \F\bMag X$, and define a \cI-indexed signature that has the following operations.
\begin{itemize}
...
Let $\equiv$ denote the binary relation obtained as above from nonemptiness of the \emph{initial} algebra for this indexed signature.
...
\begin{ex}\label{ex:poset-bifib-iso}
  Use your sequent calculus from \cref{ex:poset-bifib} to prove that in a bifibration, if $x\cong y$ in $\sM$, we have an isomorphism $x_! \cong x^*$ (that is, for any $a$ in the fiber over $y$, we have $x_!(a) \cong x^*(a)$).
\end{ex}
...
As we will see, we can still formulate canonical/atomic and focused versions of more general theories, but the canonical forms don't have as simple a description.}, as in the following derivation:
\begin{mathpar}
  \inferrule*{
...
\begin{lem}\label{thm:focused-catprod-cutassoc}
  Cut is associative and unital in the unary focused sequent calculus for categories with products.
\end{lem}
...
% TODO: Does focusing continue to identify unique canonical derivations in more complicated theories (with both positives and negatives)?  Right now I think the answer is no, that we still need commuting conversions, and that that's also true for canonical/atomic calculi.
...
\begin{figure}
  ...
  \begin{mathpar}
    \inferrule{\types X\type}{x:X\types x:X}\;\idfunc\and
    \inferrule{f\in \cG(A,B) \\ x:X\types M:A}{x:X\types f(M):B}\;fI\and
    \inferrule{x:X \types M:\emptyt \\ \types C\type}{x:X \types \abort(M):C}\;\emptyt E\\
...
\begin{figure}
  ...
  \begin{mathpar}
    \inferrule{\types X\type}{x:X\types x\atom X}\;\idfunc\and
    \inferrule{f\in \cG(A,B) \\ x:X\types M\can A}{x:X\types f(M)\can B}\;fI\and
    \inferrule{x:X \types M\atom \emptyt \\ \types C\type}{x:X \types \abort(M)\atom C}\;\emptyt E\\
    \inferrule{x:X\types M\can A}{x:X\types \inl(M)\can A + B}\;\plusI1\and
    \inferrule{x:X\types N\can B}{X\types \inr(N)\can A + B}\;\plusI2\and
    \inferrule{u:A\types P\can C \\ v:B\types Q\can C \\ x:X\types M\atom A + B}{x:X\types \case(M,u.P,v.Q)\atom C}\;\plusE\and
    \inferrule{B\in\cG\\ x:A \types M\atom B}{x:A\types M\can B}\;\atomcan
  \end{mathpar}
