...
This completes the definition of our \textbf{canonical/atomic natural deduction for categories with products}.
...
In particular, hereditary substitution implies that the usual natural deduction rules are also admissible for canonical terms.
...
\begin{proof}
  We induct on the derivation of $\types A\type$.
  \begin{itemize}
  \item If $A\in\cG$ is a base type, then we can apply $\atomcan$ to derive $x:A \types x\can A$, so $\hid{A}(x)=x$.
  \item If $A=\unit$, then $x:\unit \types \ttt\can \unit$, so $\hid{\unit}(x)=\ttt$.
  \item If $A=A_1\times A_2$, then by induction we have $y:A_1 \types \hid{A_1}(y)\can A_1$ and $z:A_2 \types \hid{A_2}(z)\can A_2$.
    ...
    \[ x:A \types \pair{\hid{A_1}(y)\hsub{\pi_1(x)/y}}{\hid{A_2}(z)\hsub{\pi_2(x)/z}} \can A;\]
    thus $\hid{A_1\times A_2}(x) = \pair{\hid{A_1}(y)\hsub{\pi_1(x)/y}}{\hid{A_1}(z)\hsub{\pi_2(x)/z}}$.\qedhere
  \end{itemize}
\end{proof}
...
The rules with annotations for this \textbf{focused sequent calculus for categories with products} are shown in \cref{fig:catprod-focused-seqcalc}.
...
\begin{figure}
  ...
  \end{mathpar}
  \caption{Focused sequent calculus for categories with products}
...
[TODO:
\begin{itemize}
\item How hard is it to prove identity, cut, and initiality directly for the focused case?  So far I've failed to prove identity by a straightforward induction rather than by actually inspecting the structure of the type ``all the way down''.
\item Does focusing continue to identify unique canonical derivations in more complicated theories (with both positives and negatives)?  Different sources have implied different answers to me; right now I think the answer is no, that we still need commuting conversions, and that that's also true for canonical/atomic calculi.
\item At some point, type-theoretic treatments of focusing tend to introduce ``polarized connectives'' as well, which is not (I think) something I want to get into here, as its connection with the category theory is even more tenuous.]
\end{itemize}
...
\begin{ex}\label{ex:catprod-subassoc}
  Prove \cref{thm:catprod-subassoc}.
\end{ex}
