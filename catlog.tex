...
\date{\today}
\makeatletter
\autodefs{\bSet\bPoset\bRelGr\bCat\bGr\bmSLat\ftype\bPrCat\bMCat\bMGr\bMPos\bRelMGr\bMonPos\bMonCat\bSMPos\bSMC\bProp}
\def\tr{\mathrm{tr}}
\let\sect\section
...
Once again we let the reader complete the argument (\cref{ex:focused-catprod-cutassoc,ex:focused-catprod-initial}).
...
Note that these rules are exactly the rules of the type theory for posetal props \cref{sec:proppos-smpos}, with the additional annotation by pre-terms.
...
Now the $g$-pre-terms introduced by the actual rule that leads to this conclusion are disjoint from the $f$-pre-terms that would be introduced by this $f$-generator rule.
...
\begin{thm}\label{thm:prop-initial}
  The free prop on a polygraph can be presented using this type theory: its morphisms $\Gamma\to\Delta$ are the \emph{lists of pre-terms} such that $\Gamma\types (M_1,\dots,M_n \mid Z_1,\dots,Z_p):\Delta$ is derivable.
...
If $M$ is an object of a prop, a \textbf{dual} of $M$ is an object $M^*$ with morphisms $\eta:()\to (M,M^*)$ and $\ep:(M^*,M)\to ()$ such that $\ep \circ_{M^*} \eta = \idfunc_M$ and $\ep \circ_{M} \eta = \idfunc_M^*$.
(In a symmetric monoidal category this reduces to the usual notion of dual.)
If $M$ has a dual $M^*$, and $f:M\to M$, the \textbf{trace} of $f$ is the composite 
\[ \tr(f) = \ep \circ_{M,M^*} (f \circ_M \eta) \]
or equivalently $(\ep \circ_M f)\circ_{M,M^*} \eta$.
In the type theory for props, the axioms of a dual say
\begin{mathpar}
  x:M \types (\eta_1 \mid \ep(\eta_2,x)) \equiv x :M\and
  y:M^* \types (\eta_2 \mid \ep(y,\eta_1)) \equiv y :M^*
\end{mathpar}
while the trace is $(\,\mid \ep(\eta_2,f(\eta_1)))$.

Now a classical fact about the trace is that it is \emph{cyclic}: if $M$ and $M'$ both have duals $M^*$ and $(M')^*$, and $f:M\to M'$ and $g:M'\to M$, then $\tr(gf) = \tr(fg)$.
...
= \tr(fg).
\]
Here $\eta,\ep$ exhibit the dual of $M$, while $\eta',\ep'$ exhibit the dual of $M'$.
The first and last equality are by definition;
the second applies the first duality equation for $M'$ with $x=f(\eta_1)$; and the third applies the first duality equation for $M$ with $x=g(\eta'_1)$.
