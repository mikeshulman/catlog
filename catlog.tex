...
\newtheorem*{rep@theorem}{\rep@title}\newcommand{\newreptheorem}[2]{%
  \newenvironment{rep#1}[1]{%
    ...
    \begin{rep@theorem} }%
    {\end{rep@theorem} } }
\newreptheorem{ex}{Exercise}
\def\exaccum{}
\NewEnviron{ex}[1]{\begin{@ex}\label{#1}\BODY\end{@ex}%
  \def\name{#1}%
  %\edef\next{\noexpand\g@addto@macro\noexpand\exaccum{\noexpand\begin{repex}{\name}\BODY\noexpand\end{repex}}}%
  %\next
  \g@addto@macro\exaccum{\begin{repex}{#1}}
  \ea\g@addto@macro\ea\exaccum\ea{\BODY}
  \g@addto@macro\exaccum{\end{repex}}
}{}
...
\begin{rmk}\label{rmk:admissible-derivable-2}\label{rmk:free}
  ...
  We will see in \cref{sec:presentations} that we can still use type theory to ``present'' categorical objects that are not themselves free (at least, not in the usual sense); but the syntax of types and terms/derivations must still itself be freely generated.
\end{rmk}


\subsection*{Exercises}

\begin{ex}{ex:categories-over}
...
\begin{ex}{ex:lattices-invertible}
...
\begin{ex}{ex:catprod-functor-uniq}
  Suppose we have
  \begin{mathpar}
    f\in\cG(A,B)\and
    g\in\cG(A,C)\and
...
\begin{ex}{ex:catcoprod-functor-uniq}
...
\begin{ex}{ex:cat-prod-coprod-uniq}
  Suppose we have
  \begin{mathpar}
    f\in\cG(A,C)\and
    g\in\cG(A,D)\and
...
\begin{ex}{ex:repmulticat-moncat}
...
\begin{ex}{ex:moncat-multisubadm}
...
\begin{ex}{ex:moncat-context-splitting}
...
\begin{ex}{ex:debruijn-context-concat}
...
\begin{ex}{ex:moncat-prod-coprod-context}
...
\begin{ex}{ex:cartmulti-oneplace}
...
\begin{ex}{ex:cocartesian-typetheory}
...
\begin{ex}{ex:semicart-relevance-strucadm}
...
\begin{ex}{ex:tree-quotient}
  Prove \cref{thm:tree-quotient}.
\end{ex}

\begin{ex}{ex:infop-ac}
