...
Now that we understand the simple type theories of multicategories and monoidal categories, let's add products and coproducts back in.
For the most part this just consists of ``putting together'' the rules from \cref{sec:multicat-moncat,sec:catprod,sec:catcoprod}, although there is a little extra work to generalize the rules for products and coproducts to the non-unary case.
...
Then we say an \fS-multicategory is \textbf{closed} if for each pair of objects $A$ and $B$ there is a specified object $A\linhom B$ and a morphism $\chi : (A\linhom B,A) \to B$ postcomposition with which defines bijections
\[ (\chi\circ_1 -): \cM(C_1,\dots,C_n;A\linhom B) \toiso \cM(C_1,\dots,C_n,A;B) \]
...
For the rest of this section, let \fS be one of the four possibilities above.
All our type theories will then include the appropriate primitive structural rules, according to the above table.
Since we are in the posetal world, we can consider sequent calculus as well as natural deduction; we start with the former.

\subsection{Sequent calculus}
\label{sec:logic-seqcalc}

[We've seen most of the connectives before, except $\linhom$.]

[TODO: After initiality, mention $\mathbf{2}$ as a particular poset, also Heyting algebras, etc.]
