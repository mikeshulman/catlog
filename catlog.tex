...
We say that an object $1$ of a multicategory is \textbf{terminal} if for any $A_1,\dots, A_n$ there is a unique morphism $(A_1,\dots, A_n)\to 1$.
...
\begin{proof}
  ...
  Conversely, suppose $1$ is terminal.
  Then in particular, we have a unique morphism $\chi : ()\to 1$, and acting on $\chi$ by the injection $\emptyset \to \{1,\dots,n\}$ can only yield the unique morphism $(A_1,\dots,A_n)\to 1$.
  Now we have to show that
  \[ (-\circ_{n+1} \chi) : \cM(A_1,\dots,A_n,1,B_1,\dots,B_m; C) \to \cM(A_1,\dots,A_n,B_1,\dots,B_m; C) \]
  is a bijection.
...
Similarly, we say that a \textbf{product} of $A$ and $B$ in a multicategory is an object $A\times B$ with projections $A\times B \to A$ and $A\times B\to B$, composing with which yields bijections
...
\begin{ex}\label{ex:distribcat}
  ...
  Recall that a \textbf{distributive category} is one with finite products and coproducts and such that the canonical maps $A\times(B+C) \to (A\times B)+(A\times C)$ are isomorphisms.
  Prove:
  \begin{enumerate}
  \item The underlying cartesian multicategory of a distributive category has coproducts.
  \item If a cartesian multicategory is representable and has coproducts, then the cartesian monoidal category it induces is distributive.
  \item These constructions define an equivalence of categories.
  \end{enumerate}
\end{ex}
