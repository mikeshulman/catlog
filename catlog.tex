...
\date{\today}
\makeatletter
\autodefs{\bSet\bMag\bVect\bPoset\bRelGr\bCat\bGr\bmSLat\ftype\bPrCat\bCoprCat\bMCat\bRepMCat\bMGr\bMPos\bRelMGr\bMonPos\bMonCat\bSMPos\bSMC\bProp}
\def\tr{\mathrm{tr}}
\let\sect\section
...
\end{tabular}
\end{center}
Note that these rules refer only to single transpositions, projections, and duplications, rather than arbitrary functions in \fS.
This is similar to how cut is usually stated in type theory in $\circ_i$-style rather than as a multi-composition.
As in that case, the smaller operations suffice to generate the more general ones (c.f.\ \cref{ex:club-generators}).
In this section we will use the above standard versions.
...
[TODO: Later on, when making these rules admissible, we may want to use their general versions, just as the multi-cut sometimes simplifies cut-admissibility.]
...
\begin{figure}
  ...
  \inferrule{
    ...
    \and
    \inferrule{\Psi\types \zero}{\Gamma,\Psi\types C}\;\zero E
    \\
...
The cut-admissibility theorem now has a new wrinkle.
Let us first consider the straightforward cases.
...
\begin{proof}
  ...
  (It does matter that we have formulated the admissible cut rule \emph{with} $A$ in the middle of the context rather than on one side; otherwise the induction would fail to go through here.)
...
\end{equation*}
Then we could apply exchanges to pair up the two copies of each type in $\Psi$, and finally a contraction on each of them to eliminate the duplicates.
...
Moreover, the solution sketched there (inducting first on types and then on derivations) does not work either, since the types are not changing.
...
In our case, the mix rule says that if we have derivations of $\Psi\types A$ and $\Gamma\types B$, where $\Gamma$ contains one or more copies of $A$, then we can construct a derivation of $\Gamma^A,\Psi\types B$, where $\Gamma^A$ is $\Gamma$ with one or more copies of $A$ removed.
...
[TODO: Mention $\mathbf{2}$ as a particular poset, also Heyting algebras, etc.]

[TODO: Also mention the traditional syntax for natural deduction.]
