...
Now that we have a nice notation to distinguish between derivations, we should observe that \cref{thm:categories-cutadm} is not just a statement about derivability.
...
Thus, we refer to the operation on terms defined by \cref{thm:categories-cutadm} as \textbf{substitution}.
...
The proof of \cref{thm:categories-cutadm} \emph{defines} the notion of substitution recursively in the following way:
\begin{align*}
  y[M/y] &\coloneqq M\\
  f(N)[M/y] &\coloneqq f(N[M/y])
\end{align*}
When terms are regarded as syntax in their own right rather than as mere proxies for derivations, it is common to define substitution as an operation on terms first, and then to state (the analogue of) \cref{thm:categories-cutadm} as ``if $x:A\types M:B$ and $y:B\types N:C$ are derivable, then so is $x:A\types M[N/y]:C$''.
...
\end{thm}
\begin{proof}
  Let $\F\bCat\cG$ be defined as in the statement, with composition given by substitution constructed as in \cref{thm:categories-cutadm}.
...
\emph{that yield elements in canonical form}, eliminating (or at least reducing) the need for quotients.
(Starting in \cref{sec:catprod} we will see that more complicated theories generally still need an equivalence relation on terms beyond $\alpha$-equivalence, but it is arguably still simpler than the quotient required for a na\"ive presentation of free structures.)

A similar way to say this is that categorical logic is about \emph{showing that two different categories have the same\footnote{Of course, technically, an object of one category is not generally also an object of another one.  So what we mean is that there is an easy way to transform the initial object of one category into the initial object of another.} initial object}.
...
It is much more convenient to describe the reduction rules using a term syntax for derivations, as introduced in \cref{sec:category-cutadm}.
...
As with \cref{thm:categories-cutadm}, this proof can be regarded as \emph{defining} what it means to ``substitute'' $M$ for $y$ in $N$.
