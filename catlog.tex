...
In fact, in the simple case of categories with products, there are tricks enabling us to eliminate \emph{both} kinds of redundancy, and thereby do without any $\equiv$ for both sequent calculus and natural deduction.
...
Thus, we focus in this section on theories with $\equiv$, relegating the tricks to the ``chapter appendices'' \cref{sec:catprod-atomcan,sec:catprod-focusing}.
...
[TODO: Introduce pairing, projection.]
...
For this reason, except for \cref{sec:catprod-focusing}, we will henceforth only use sequent calculus for posetal theories.
...
Note that in addition to the $\beta$- and $\eta$-reductions, we assert reflexivity, symmetry, and transitivity, and also that all the previous rules preserve equality.
...
[TODO: Relegate these next two sections to something more appendix-like.  Can we have a ``chapter appendix''?]
