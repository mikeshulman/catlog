\documentclass{book}
\usepackage{mathpartir,cancel,cmll,textcomp,version}
...
\date{\today}
\makeatletter
\autodefs{\bSet\bMag\bVect\bPoset\bRelGr\bCat\bGr\bmSLat\ftype\bPrCat\bCoprCat\bMCat\bRepMCat\bMGr\bMPos\bRelMGr\bMonPos\bMonCat\bSMPos\bSMC\bProp\bTV\bHeyting}
\def\tv{\mathbf{2}}
\def\tr{\mathrm{tr}}
\let\sect\section
...
\def\toE{\mathord{\to}E}
% 
\excludeversion{fat}            % fat multicategories
\makeatother
\begin{document}
\maketitle
...
However, it is universally the non-variable part $M$ that is called the \emph{term}, and the separation into variable (or, later, variables) and term is responssible for much of the characteristic behavior of terms in type theory.
...
We call this the \textbf{de Bruijn method} (in \cref{chap:simple} we will generalize it to the methods usually indicated by that name).

The second method is to allow arbitrary choices of variable names (perhaps taken from some standard alphabet), but be aware of the operation of variable renaming.
In our simple case, the important properties of this operation are summarized in the following lemma.

[TODO: Get rid of all explicit discussion of $\alpha$-equivalence; just trust the reader and refer to \cref{sec:alpha}.]

\begin{lem}\label{thm:category-alpha}
  If $x:X\types M:A$ is a term notation for a particular derivation in the cut-free type theory for categories under \cG, then so is $y:X\types M\alsub{y/x}:A$ for any other variable $y$, where $M\alsub{y/x}$ denotes the result of replacing $x$ by $y$ wherever it appears in $M$.
  Moreover, if $x:X\types M:A$ and $y:X\types N:A$ are two term notations for the same derivation, then $N=M\alsub{y/x}$.
\end{lem}
\begin{proof}
  The first sentence is proved by induction on the derivation.
  If it consists of $\idfunc$, so that $A=X$ and $M=x$, then the same rule gives $y:X\types y:X$.
  And if it ends with a generator rule, we simply induct.

  The second sentence likewise follows by induction: the generator rule cannot introduce any new redundancy in terms, whereas the redundancy in the $\idfunc$ rule is exactly the choice of variable name.
\end{proof}

If $N=M\alsub{y/x}$ (or equivalently $M=N\alsub{x/y}$), we say that $M$ and $N$ are \textbf{$\alpha$-equivalent}; \cref{thm:category-alpha} shows that this is an equivalence relation on terms whose quotient is isomorphic to the set of derivations.
Thus, we can say that a derivation determines a unique $\alpha$-equivalence class of terms.

Of these two methods, the de Bruijn method is theoretically cleaner, and better for implementation in a computer, but tends to detract from readability for human mathematicians (see below).
...
Note that, like $M\alsub{y/x}$, the operation $N[M/y]$ this is ``meta-notation'': the square brackets are not part of the syntax of terms, instead they denote an operation \emph{on} terms.
(However, while $M\alsub{y/x}$ is an operation only on terms, leaving the underlying derivation unchanged, $N[M/y]$ represents an underlying operation on derivations.)
...
Note, though, that because a derivation is represented by a term together with a variable for the antecedent, technically the operation on derivations has to track the variables too, whereas the notation $N[M/y]$ represents only the term part.
Thus, the definitions~\eqref{eq:category-sub-1} and~\eqref{eq:category-sub-2} are only complete when combined with the statement that the antecedent variable of $N[M/y]$ is the antecedent variable of $M$.
...
[TODO: State other initiality theorems using ``derivations'' rather than ``derivable terms''.]
...
\begin{princ}\label{princ:term-der-alpha}
  A rule applies equally to any derivations of its premises, but the abstract-variable term notation for that rule may require certain compatibilities between the abstract-variable terms of the premises, which can always be ensured by $\alpha$-equivalence.
\end{princ}
...
It is also straightforward to prove analogues of \cref{thm:category-tad,thm:category-alpha}.
...
Recall from \cref{sec:catprod} the general principle (\cref{princ:term-der-alpha}) that a term notation should indicate derivations of the premises of the rule by including terms for them, and that in general this can be tricky when using abstract variables because a derivation is only determined by a term together with its variable.
...
We say they are \textbf{bound} variables, like the integration variable in a definite integral.
By contrast, the antecedent variable $x$ is \textbf{free}.
Textually each bound variable is associated to a subterm called its ``scope'', delimiting the places where it can be referred to;
...
However, because of the potential confusion, it is arguably bad mathematical style to use the same name for distinct variables, even if there is technically no ambiguity.
...
Thus, from now on we will adhere to the \emph{opposite} of the de Bruijn method.

\begin{conv}[Variable convention]
  All variables occurring in any term, including bound ones, will be assumed to be distinct.
\end{conv}

Technically, it requires proof that it is always possible to represent a derivation by a term satisfying this convention.
For this purpose we have to extend our notion of $\alpha$-equivalence to include the renaming of bound variables; this is easy as long as all our terms already adhere to the convention.

\begin{lem}\label{thm:catcoprod-alpha}
  Suppose $x:X\types M:A$ represents a particular derivation in the unary type theory for categories with coproducts that adheres to the variable convention.
  Let $y$ be a variable distinct from $x$ (which might appear bound in $M$), and let $z$ be a variable not occurring in $M$ at all.
  Then $z:X\types M\alsub{z/x}:A$ and $x:X\types M\alsub{z/y}:A$ both also represent the same derivation (and adhere to the variable convention).
  Moreover, two terms represent the same derivation precisely when they are related by a sequence of such operations.
\end{lem}
\begin{proof}
  By induction on derivations, as before; in all cases the only interesting rules are $\idfunc$ and $\plusE$.
  In the case of $z/x$, the $\idfunc$ rule $x:X\types x:X$ can be replaced by $z:X\types z:X$.
  And for $\plusE$ with a term $x:X\types \case(M,u.P,v.Q)$, our variable convention ensures that $u$ and $v$ are distinct from $x$, and $x$ cannot occur in $P$ or $Q$ (since it is not the free variable, and is not a bound variable anywhere by the variable convention).
  Thus, we can inductively consider only $x:X\types M:A+B$.

  In the case of $z/y$, there is nothing to do for $\idfunc$ since $y$ is distinct from (hence does not occur in) $x$.
  For $\plusE$ with a term $x:X\types \case(M,u.P,v.Q)$, we have to compare $y$ with $u$ and $v$.
  If $y=u$, then we inductively apply the first sort of renaming (of a free variable rather than a bound one) in $u:A\types P:C$, and similarly if $y=v$; in neither case can $y$ also appear in the other one or in $M$, by our variable convention.
  And if $y\neq u$ and $y\neq v$, then we inductively rename the bound variable $z/y$ in $M$, $P$, and $Q$ (though by the variable convention, $y$ can occur in at most one of them).

  For the last statement, if the derivation is $\idfunc$, then for any two terms $x:X\types M:A$ and $z:X\types M':A$ representing it we must have $M=x$ and $M'=z$, whence a renaming $z/x$ relates them.
  And if the derivation ends with $\plusE$ and we have terms $x:X \types \case (M,u.P,v.Q)$ and $z:X\types \case(M',a.P',b.Q')$, then inductively $x:X\types M:A+B$ and $z:X\types M':A+B$ are related by a sequence of renamings, and likewise so are $u:A\types P:C$ and $a:A\types P':C$ and also $v:B\types Q:C$ and $b:B\types Q':C$.
  [TODO: Intermediate?  Variables might be ``switched''.]
  Our variable conventions ensure that the renamings leading from $M$ to $M'$, from $P$ to $P'$, and from $Q$ to $Q'$ can all be applied to the overall $\case$ terms without modifying the other subterms, so putting them together we obtain the desired result.
\end{proof}

\begin{lem}\label{thm:catcoprod-varconv}
  Every derivation can be represented by a term that adheres to the variable convention.
\end{lem}
\begin{proof}
  By induction;
  whenever we have to combine multiple terms into one, we first apply \cref{thm:catcoprod-alpha} to make all their variables distinct.
\end{proof}
...
\begin{proof}
  ...
  \end{align*}
  There is one thing to be mentioned: even if the terms $M$ and $N$ both adhere to our variable convention, it might be that some of the bound variables in $N$ also appear in $M$ (either as bound variables or as the free variable $x$).
  Therefore, to ensure that the result of substitution $N[M/y]$ also adheres to the variable convention, we may have to modify $M$ or $N$ by an $\alpha$-equivalence, which \cref{thm:catcoprod-alpha} ensures is possible.
\end{proof}
...
\begin{thm}\label{thm:catcoprod-initial}
  For any directed graph \cG, the free category with coproducts generated by \cG can be described by the unary type theory for categories with coproducts under \cG: its objects are the derivations of $\types A\type$, and its morphisms $A\to B$ are the derivations of $A\types B$ (or the derivable term judgments $x:A \types M:B$ modulo $\alpha$-equivalence), modulo the equivalence relation $\equiv$.
\end{thm}
\begin{proof}
...
% \item For any $f\in \cM(A,b)$, with $A:L\to \cM_0$, we have
%   \begin{mathpar}
%     \idfunc_b \circ \{f\} = f\and
%     f\circ \idfunc_A = f
%   \end{mathpar}
%   where $\idfunc_A$ denotes the $L$-indexed family of morphisms picking out $\idfunc_{a_l}$ for each $l\in L$.
% \item For any $h,G,F$ we have
%   \begin{mathpar}
%     (h\circ G) \circ \bigcup F
%     h \circ (G\circ F)
%   \end{mathpar}
\end{enumerate}
...
\begin{ex}\label{ex:fat-multicats}
  Fill in the details in the two definitions of ``fat multicategories'' from \cref{sec:fat-multicats} (using multi-composition and one-place composition), and prove that both categories of fat multicategories are equivalent to the category of ``thin'' multicategories.
\end{ex}
\end{fat}
...
(This is an instance of \cref{princ:term-der-alpha} that while rules apply to all derivations equally, the term notation for them can require applying $\alpha$-equivalences to some premises for compatibility.
In \cref{sec:catprod} ``compatibility'' meant using the \emph{same} variable, while here it means using \emph{different} variables.)

\begin{fat}
\begin{rmk}
  Renaming variables for context concatenation can be formulated elegantly with the setup that we used for fat multicategories in \cref{sec:fat-multicats}.
  Recall that \fL denotes the category of finite linear orders and isomorphisms; now define formally a \textbf{context} to be an $L$-indexed family of types (i.e.\ derivable judgments $\types A\type$) for some $L\in\fL$, which we might write $\Gamma:L\to \ftype$.
  The elements of $L$ are the \textbf{variables}.
  ...
  But of course, in many cases it is not be necessary to rename \emph{all} the variables, only those that would otherwise conflict; just as when constructing a coproduct of sets we don't need to ``tag'' all the elements of both sets, only those belonging to their intersection.
\end{rmk}
\end{fat}
...
When applying a rule ``downwards'', this applies to its premises, while when applying it ``upwards'', this applies to its conclusion (recall \cref{rmk:context-splitting-1}).
...
The terms where we have to rename variables and so on are only a \emph{notation} for the real mathematical objects, namely derivations.
...
It is useful when proving the basic properties of terms, as follows:
...
\begin{lem}\label{thm:multicat-alpha}
  If $\Gamma,x:A,\Delta\types M:B$ is a term notation for a particular derivation in the cut-free type theory for multicategories under \cG, and $y$ is a variable not occurring in $\Gamma,\Delta$, then $\Gamma,y:X,\Delta\types M\alsub{y/x}:A$ is also a notation for the same derivation.
  Moreover, if $\Gamma\types M:B$ and $\Gamma'\types M':B$ are two term notations for the same derivation, then they are related by a sequence of such renamings.
\end{lem}
\begin{proof}
  If the derivation ends with $\idfunc$, then we must have $M=x$ and $\Gamma=\Delta=()$, whence $y:A\types y:A$ is also derivable.
  And if $M=f(N_1,\dots,N_n)$, then by linearity $x$ must occur in exactly one of $N_1,\dots,N_n$, say $N_k$, so that $\Gamma_k \types N_k:A_k$ is $\Psi,x:A,\Delta\types N_k:A_k$.
  Then we can inductively represent that derivation by $\Psi,y:A,\Delta\types N_k\alsub{y/x}:A_k$ and then apply $f$ again.

  For the last statement, the only redundancy in terms comes from the $\idfunc$ rule, so if we rename all the variables in $\Gamma'$ to the corresponding variables in $\Gamma$ then the resulting terms must also coincide.
\end{proof}
...
The rule $\tensorI$, like the rule $\timesI$ from \cref{sec:catprod}, ``pairs up'' two derivations of $\Gamma\types A$ and $\Delta\types B$, thus must include terms notating both.
...
But \emph{some} of the variables in $N$ do remain in the context of the conclusion (and must be disjoint from those in $M$, by $\alpha$-equivalence if necessary), so we only need to ``bind'' the other two variables $x$ and $y$.
...
There is one new thing going on relative to \cref{sec:catcoprod}: because the context can now contain multiple variables, the term $N$ can contain \emph{free} variables as well (those appearing in the contexts $\Gamma$ and $\Delta$), in addition to the free variables appearing in the term $M$ (those appearing in the context $\Psi$).
...
We maintain the variable convention introduced in \cref{sec:catcoprod} that no two variables (free or bound) in any term are the same.
As there, this sometimes requires additional renaming variables when applying rules to terms: for instance, even if $\Gamma$ and $\Delta$ in $\tensorI$ involve disjoint variables, there could be bound variables in $M$ that also appear free in $\Delta$ or vice versa unless we apply a renaming.

\begin{lem}\label{thm:moncat-alpha}
  Suppose $\Gamma,x:A,\Delta\types M:B$ is a term notation for a particular derivation in the simple type theory for monoidal categories under \cG, adhering to the variable convention.
  Let $y$ be a variable not occurring in $\Gamma,\Delta$, and let $z$ be a variable not occurring in $M$ at all.
  Then $\Gamma,z:X,\Delta\types M\alsub{z/x}:A$ and $\Gamma,x:X,\Delta\types M\alsub{z/y}:A$ are also notations for the same derivation.
  Moreover, if $\Gamma\types M:B$ and $\Gamma'\types M':B$ are two term notations for the same derivation, then they are related by a sequence of such renamings.
\end{lem}
\begin{proof}
...
In the proof of \cref{thm:multiposet-cutadm} we remarked that sometimes we have to rename the variables appearing in one of the terms before doing a substitution, to ensure that the variables in the context of the substituted term are all distinct; but that this essentially happens automatically when we regard substitution as acting on derivations.
Something else similar happens here in the rule $\match_\tensor(N,uv.P)[M/x] = \match_\tensor(N,uv.P[M/x])$: we must ensure (by $\alpha$-equivalence if necessary) that the bound variables $u$ and $v$ do not appear in $M$.
(This didn't arise in the unary type theory of \cref{sec:catcoprod} because there it was not possible to substitute into the subterms $u.P$ and $v.Q$ of a $\case$ term containing bound variables, since they could not contain any \emph{other} variables to be substituted for.)
One generally says that this is because otherwise, a $u$ or $v$ in $M$ would after substitution be ``in the scope'' of the binding of $u$ or $v$, whereas all the variables of $M$ ought to be free in the substituted term.
When we regard substitution as an operation on derivations, the point is just that to eliminate a cut after $\tensorE$ of the following sort:
\begin{mathpar}
  \inferrule*[Right=cut]{
    \Gamma\types M:A \\
...
When one tries to be very precise about syntax with named variables, this sort of ``capture-avoiding substitution'' is both necessary and tedious.
The de Bruijn methods avoid it, but at a fairly severe cost to readability as mentioned above.
But from our perspective, this is all just an artifact of how we choose to represent derivations by terms involving abstract variables.
When substitution is an operation on derivations, there are no variables to ``capture'' and nothing to worry about.
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \\
    \inferrule{ }{\Gamma\types \ttt(\Gamma):\unit}\;\unit I
    \and
...
\end{mathpar}
Unlike with the annotations $\Gamma|\Delta$ on $\match$es (see \cref{ex:moncat-context-splitting}), these terms really can represent distinct morphisms. % in a distributive monoidal category with products.
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \pi_2(\pair M N) \equiv N\and
    \pair{\pi_1(M)}{\pi_2(M)} \equiv M\and
    \ttt(\Gamma) \equiv M \and
...
[TODO: $\alpha$-equivalence and binding]
...
\begin{lem}\label{thm:stlc-strucadm}
  ...
  Moreover, they make the derivations into a cartesian multigraph.
\end{lem}
\begin{proof}
  TODO.
\end{proof}
...
Let us return to the posetal type theory of Heyting algebras, but with the rules formulated as in \cref{thm:cart-constctx}.
...
\end{equation}
One way to deal with this is to break the bijection between terms and deductions, in a way that enables us to represent both of the above two derivations by the same term.
...
Note that unlike the $\beta$- and $\eta$-conversions, the equality~\eqref{eq:prop-bad-terms} is not directional: it makes no sense to regard one or the other side as ``simpler'' or ``more canonical'' than the other, and it is likewise unclear how to design a type theory in which one would be permitted but not the other.
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \inferrule{
      ...
      \mid \tau\left({ g}_1^{\fb_1}(\vec P_1),\dots,{ g}_m^{\fb_m}(\vec P_m),\vec Z\right)\right)
      :\Delta
    }
  \end{mathpar}
...
The latter is all hidden in the notation $\vec f$, and indicated in a couple of places by double subscripts $D_{11}, D_{21},\dots$
...
The reason for the extra strength of~\ref{item:terms-uniq}, and for condition~\ref{item:term-wf}, is to make type-checking a ``deterministic recursive algorithm'', as follows.
...
Otherwise, axiom~\ref{item:term-wf} ensures that the algorithm must terminate (by bottoming out at nullary rules), and we have now constructed a derivation represented by the term $t$.
...
Notationally, we can specify a signature along with a term syntax for it by annotating its rules by terms.
...
What we have in mind is that the initial $\sig$-algebra will supply the set of terms in a term syntax for some other signature, with the operations of $\sig$ corresponding to the term notations for the rules in that otherh signature.
...
Now we have the initial $\sig$-algebra $W\sig$.
We define, by recursion, an action of $\mathrm{Aut}(\dA)$ on $W\sig$: for a permutation $\sigma$ we set
\begin{align*}
  \sigma \cdot \act{v(x)} &= \act{v(\sigma(x))}\\
  \sigma \cdot \act{b(x)}(M) &= \act{b(\sigma(x))}(\sigma\cdot M)
\end{align*}
...
Because all operations in $\sig$ have finite arity\footnote{If $\sig$ were allowed to contain infinitary operations, then to make this work, the cardinality of \dA would have to be of cofinality greater than any of their arities.} and all trees in $W\sig$ are well-founded, only finitely many variables can occur in any tree (either through $v$ or $b$).
Since \dA is infinite, for any $t\in W\sig$ there is always some variable $z\in \dA$ that does not occur in $t$.
We write $z\notin t$ to mean that $z$ does not occur in $t$.
...
We include a ``reflexivity at variables'' $v(x)\equiv v(x)$, the operations making $\equiv$ a congruence for all operations of $\sig$, and also one further operation that in rule form looks like this:
\begin{equation}
  \inferrule{z\notin t \\ z\notin s \\ z\neq x \\ z \neq y \\ (zx)\cdot t \equiv (zy)\cdot s}
  {b(x)(t) \equiv b(y)(s)}\label{eq:alpha-gen}
\end{equation}
Here $(zx)$ and $(zy)$ denote the transposition permutations that swap $z$ with $x$ and $z$ with $y$, respectively.
Since $z$ does not occur in $t$ and $s$, the permutation actions $(zx)\cdot t$ and $(zy)\cdot s$ amount to replacing all occurences of $x$ in $t$ and $y$ in $s$ (even bound ones) by $z$.
The rule then says that if these two results are $\alpha$-equivalent, then so are the terms $x.t$ and $y.s$ with new bound variables.
...
Unlike in \cref{sec:axioms}, we do not explicitly include operations making $\equiv$ symmetric and transitive.
However, we can prove that it is using an induction (this is itself a sort of cut-admissibility).
Thus, it is an equivalence relation.
Omitting symmetry and transitivity has the good consequence that~\eqref{eq:alpha-gen} is invertible: if $x.M\equiv y.N$, then since~\eqref{eq:alpha-gen} is the only rule that can produce such an output, it must be that $(zx)\cdot M \equiv (zy)\cdot N$ for a fresh $z$.

The quotient $W\sig/\equiv$ is, of course, our set of ``terms modulo $\alpha$-equivalence of bound variables''.
Since $\equiv$ is a congruence for all the operations of $\sig$, these all descend to the quotient.
In particular, we have induced variable binding operations on the quotient, which we also denote by $x.M$.
...
For instance, in unary type theories we have said that a term judgment such as $x:A\types M:B$ can be read as $x:M: (A\types B)$.
...
Similarly, in a simple type theory the terms for $\Gamma\types B$ should have $n$ variable bindings outermost, where $n$ is the length of $\Gamma$ (this is why in \cref{sec:terms} we allowed different judgments to have different sets of potential terms).
...
Let us consider $\case$ as a paradigmatic example.
Since the rule $\plusE$ has three premises, what we have to give is an operation $\dT\times \dT\times \dT \to \dT$.
...
By the above remarks, we can do this so that $x$ does not appear in $u.P$ or $v.Q$ (which have only finitely many variables each).
Now we can write $x.\case(M,u.P,v.Q)$; but for this to make sense we have to check that it is independent of the chosen representatives.
For $u.P$ and $v.Q$ this is easy since $\equiv$ is a congruence for all operations of $\sig$, including $\case$.
And if $x.M\equiv y.N$, then as remarked above we have $(zx)\cdot M \equiv (zy)\cdot N$ for some $z$, which we may also take to not appear in $u.P$ or $v.Q$.
Thus we have
\begin{align*}
  x. \case(M,u.P,v.Q)
...
In more complicated cases, we also need to poke down into all the terms to ensure that certain variables in their context are disjoint or equal.
For instance, the term operation representing $\timesI$ takes as given $x.M$ and $y.N$, but its output has only one shared variable; thus we have to first note $x.M \equiv z.((zx)\cdot M)$ and $y.N \equiv z.((zy)\cdot N)$ for some $z$ appearing in neither, and then write $z.\pair{(zx)\cdot M}{(zy)\cdot N}$ for the pairing term.
Based on these examples, we trust that the reader could formulate a precise definition of all the terms used in this book as operations on $\alpha$-equivalence classes.
...
Since \dT is a subset of an initial algebra, and our operations are built using at least one operation of that algebra, \cref{defn:terms}\ref{item:term-wf} is straightforward.
Finally, the proof of \cref{defn:terms}\ref{item:terms-uniq} essentially means checking that we chose the operations of the term signature to contain enough information to reconstruct a derivation step-by-step.
