...
However we will need notions of limit and colimit in multicategories, which are less well-known, so we review them here.
...
\end{mathpar}
[TODO: ``natural deduction'' means ``arbitrary on the left'', here ``arbitrary context'', except that we have a splitting of it; that could appear on top.]
...
But in terms of derivations, the point is just that when applying a cut after $\tensorE$:
\begin{mathpar}
  \inferrule*[Right=cut]{
    \Gamma\types M:A \\
