...
we call this the \textbf{de Bruijn method}. %; it is certainly the simplest in our present case.
...
% Formally, we may introduce this using a judgment $(x:A \types M:B) \equiv (y:A \types N:B)$ generated by the following rule
% \begin{mathpar}
%   \inferrule*{x,y \text{ any variables}}{(x:A\types x:A)\equiv (y:A\types y:A)}
% \end{mathpar}
% together with the usual equivalence relation and congruence rules:
% \begin{mathpar}
%   \inferrule{x:A\types M:B}{(x:A\types M:B)\equiv (x:A\types M:B)}\and
%   \inferrule{(x:A \types M:B) \equiv (y:A \types N:B)}{(y:A \types N:B) \equiv (x:A \types M:B)}\and
%   \inferrule{(x:A \types M:B) \equiv (y:A \types N:B) \\ (y:A \types N:B) \equiv (z:A \types P:B)}{(x:A \types M:B) \equiv (z:A \types P:B)}\and
%   \inferrule{(x:A \types M:B) \equiv (y:A \types N:B) \\ f\in\cG(B,C)}{(x:A\types f(M):B)\equiv (y:A\types f(N):B)}
% \end{mathpar}
% Informally, this equivalence relation just means that
...
% Now the idea is that $\acase AB(y,f,g)$ denotes the image of $y:A+B$ under the morphism $A+B\to C$ determined by $f:A\to C$ and $g:B\to C$ (the ``coparing'', sometimes denoted $[f,g]$).
% The rule mentions an arbitrary term $M:A+B$ rather than a variable $y$ simply because it has to build in a cut.
% And we annotate it with the type $A+B$ in order to make type-checking possible (but often we will simplify it by writing just $\case$).
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \inferrule{u:A\types P:C \\ v:B\types Q:C \\ x:X\types M:A + B}{x:X\types \acase AB(M,u.P,v.Q):C}\;\plusE
  \end{mathpar}
...
% The term $\acase AB(M,u.P,v.Q)$ appearing in the rule $\plusE$, however, probably requires some explanation.
...
\subsection{Index-free multicategories}
\label{sec:indexfree-multicats}
...
More precisely, let \fL denote the category of finite linear orders and isomorphisms; since two isomorphic linear orders are uniquely isomorphic, \fL is equivalent to the discrete category \dN.
Given any set $\cX$, let $(\fL\dn \cX)$ denote the category whose objects are functions $L\to \cX$ for $L\in \fL$, and whose morphisms are morphisms in \fL forming a commutative triangle.
We define an \textbf{index-free multigraph} to consist of a set $\cG_0$ of objects together with a hom-functor $(\fL\dn\cG_0)\times \cG_0 \to \bSet$.
Since $(\fL\dn \cX)$ is equivalent to the discrete category on the set $\coprod_{n\in\dN} \cX^n$ of finite lists of elements of $\cX$ (i.e.\ the free monoid on $\cX$), index-free multigraphs are essentially equivalent to ordinary multigraphs; but they admit more naturally defined operations.

For instance, given $L\in\fL$ and $K:L\to \fL$, the disjoint union $\coprod_{l\in L} K_l$ has a lexicographic ordering making it an object in \fL: if $k_1\in K_{l_1}$ and $k_2\in K_{l_2}$, then $k_1<k_2$ in $\coprod_{l\in L} K_l$ if either $l_1<l_2$ in $L$ or both $l_1=l_2$ in $L$ and $k_1<k_2$ in $K_{l_1}(=K_{l_2})$.
This is the index-free version of concatenation of lists.

Now we can define an \textbf{index-free multicategory} to be an index-free multigraph \cM together with
\begin{enumerate}
\item For each object $A$, an identity arrow $\idfunc_A \in \cM(\{A\},A)$ where $\{A\}$ denotes a family indexed by some canonically chosen one-element linear order.
\item For any object $C\in\cM_0$, \fL-indexed family $B:L\to\cM_0$, and doubly indexed family $K:L\to \fL$ with $A_l : K_l \to \cM_0$, a composition operation
  \begin{align*}
...
The one-place operations on an index-free multicategory now take the following form: given $g\in \cM(B,C)$, where $B:L\to \cG_0$ with $L\in\fL$ and $C\in\cG_0$, and also $f\in \cM(A,B_l)$ for some $l\in L$, where $A:K\to \cG_0$ and $K\in \fL$, we have $g\circ_l f \in \cM(B\cup_l A,C)$, where $B\cup_l A:L\circ_l K \to \cG_0$ is induced by $B$ and $A$.
...
We will see later on that the difference between index-ful and index-free multicategories is roughly the same as the difference between the methods of de Bruijn and of $\alpha$-equivalence in type theory.


\subsection*{Exercises}

\begin{ex}\label{ex:multicat-defns}
  Prove that all four definitions of multicategory given in this section (multi-composition or one-place compositions, index-ful and index-free) are equivalent.
\end{ex}
...
This can be formulated elegantly using the index-free setup of \cref{sec:indexfree-multicats}.
...
Just like we have not formally defined what a ``term'' is, we have not formally defined what an ``occurrence of a variable'' in a term is.
But since our terms are really just an alternative syntax for derivation trees, we can understand \cref{thm:multicat-linear} more precisely in terms of the latter.
In that language, it says that each occurrence of a type in the antecedent of a derivable sequent can be traced back up exactly one branch of the derivation tree.
...
Now note that if we represent contexts as families of types indexed by finite linear orders, as suggested in the discussion of $\alpha$-equivalence for concatenation above, then the derivable sequents $\Gamma\types A$ form \emph{precisely} an index-free multigraph as described in \cref{sec:indexfree-multicats}, and substitution $N[M/x]$ has precisely the form of the index-free one-place composition operation ``$N\circ_x M$''.
Similarly, we have:
...
There is one new thing going on relative to \cref{sec:catcoprod}: because the context can now contain multiple variables, the term $N$ can contain variables that are \emph{not} bound, in addition to the unbound variables appearing in the term $M$.
Unbound variables are called \emph{free}; they are those that appear in the context of the term.
...
\end{mathpar}
produces a term $\match_\tensor(\tpair{f(u)}{g(v)}, xy.h(z,x,y,w))$ in which the variables $z,w$ in $h(z,x,y,w)$ are free, in addition to the free variables $u,v$ in $\tpair{f(u)}{g(v)}$.
...
We can also formulate index-free versions of such multicategories along the lines of \cref{sec:indexfree-multicats}, by replacing the category \fN by the category \fF of all finite linear orders and all functions (not necessarily order-preserving) between them, and requiring that \fS include at least the order-preserving bijections.
(This somewhat-unnatural treatment of orders is only necessary if we want to include ordinary multicategories as examples; if we only care about multicategories that are at least symmetric, we can omit the linear orders entirely and take the objects of \fF to be just finite sets.)
In \cref{sec:indexfree-multicats} we asked that the hom-sets depend functorially on isomorphisms between linear orders (which, with hindsight, we can see as a categorical analogue of $\alpha$-equivalence); now that functoriality is unified with the action of the ``interesting'' maps in \fS such as symmetries, diagonals, and projections.
...
Lest the reader think that Heyting algebras seem esoteric, we point out that the lattice of open subsets of any topological space is a Heyting algebra.
...
Moreover, at least in the linear case one should also add a phrase like ``multiplicative-additive''\footnote{In the lingo of linear logic, $\tensor$ is a ``multiplicative'' connective, while $\meet$ and $\join$ are ``additive''.
  Classical linear logic also includes another multiplicative connective called $\parr$ that is dual to $\tensor$ in the same way that $\join$ is dual to $\meet$.}, because the name ``linear logic'' usually refers to a system with some additional modalities (see [TODO]).
...
\end{mathpar}
and in fact the type theory for closed relevance monoidal lattices cannot derive $()\types A\hom (B\hom A)$ (although this is not obvious).
...
[TODO: These ``equivalent forms'' are the same ones we use to make cartesian structure admissible.  Should we postpone this discussion until then?  That would also explain why this formulation of natural deduction doesn't explicitly apply the structural rules.]

Finally, let us end this section by explaining the terminology ``natural deduction''.
We return to the cartesian case, and since $\tensor$ and $\one$ coincide here with $\meet$ and $\top$ we omit the former pair.
Moreover, in the presence of contraction and weakening, the rules $\idfunc$, $\bot E$, $\joinE$, and $\homE$ have the following equivalent forms:
\begin{mathpar}
  \inferrule{A\in \Gamma}{\Gamma\types A}\;\idfunc\and
  \inferrule{\Gamma\types \bot}{\Gamma\types C}\;\bot E\and
  \inferrule{
    \Gamma\types A\join B \\ \Gamma,A \types C \\ \Gamma,B\types C
  }{\Gamma \types C}\;\joinE\and
  \inferrule{\Gamma\types A\hom B \\ \Gamma\types A}{\Gamma\types B}\;\homE
\end{mathpar}
Let us also suppose for simplicity that $\cG$ contains no nonidentity morphisms.
Then all the rules have the property that the antecedent of the conclusion is arbitrary, while the antecedents of the premises are the same, perhaps with a new type added.
In other words, as we proceed down a derivation tree, we only ever \emph{remove} types from the context.

This leads to the following reformulation of the theory.
We omit the contexts ``$\Gamma\types$'' from all judgments in a derivation tree, writing simply the consequent $A$.
...
\begin{ex}\label{ex:cart-typetheory}
  Prove \cref{thm:semicart-multicat-repr,thm:cart-multicat-repr} in our posetal type theories.
  Specifically:
  \begin{enumerate}
...
\begin{ex}\label{ex:cocartesian-typetheory}
  Is there a well-behaved type theory (i.e.\ having admissible cut and an initiality theorem) corresponding to the ``cocartesian multicategories'' of \cref{ex:cocartesian-clubs}?
  \textit{(As of this writing, the answer is not known to the author.)}
\end{ex}


\section{Intuitionistic sequent calculus}
\label{sec:logic-seqcalc}

[Should we do this at all?  Should it be an exercise?]
...
As remarked at the end of \cref{sec:logic}, in this case we can omit $\tensor,\one$ and reformulate the remaining rules so that on our way down the derivation tree we only ever remove types from the context.
It turns out that when we annotate it by terms, this reformulation is also sufficient to make all the structural rules admissible, essentially because we can propagate them all the way up the derivation tree.

[TODO.  Should we mention NNOs and primitive recursion here?]
...
For cartesian monoidal categories, this is a (multi-sorted) algebraic theory.
