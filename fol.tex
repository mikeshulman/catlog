...
\end{lem}
\begin{proof}
  By the proof of \cref{thm:regular-subobjects}, the interpretation of $y:B\types (\exists x:A.f(x)= y)\prop$ is constructed as follows:
  \begin{enumerate}
  \item Start with the diagonal $B\to B\times B$, representing $y_1:B,y_2:B\types (y_1= y_2)\prop$.
...
\begin{lem}\label{thm:logic-mono}
  Consider the Horn theory with two types $A,B$, one morphism $f:A\to B$, and one axiom $y:B,x_1:A,x_2:A \cb (f(x_1)=y),(f(x_2)=y) \types x_1=x_2$.
  A model of this theory in a category \cS with finite limits is precisely an monomorphism in \cS.
\end{lem}
\begin{proof}
...
\begin{ex}\label{ex:regular-epis}
  A morphism in a category is a \textbf{regular epimorphism} if it is the coequalizer of some parallel pair.
  Prove that every regular epimorphsim is an extremal epimorphism.
\end{ex}
