...
So far we only have one hyperdoctrine to apply this theorem to (other than the free ones produced by type theory), namely that of \cref{eg:subset-hyperdoctrine}.
However, even this one example is not trivial: it implies the \emph{soundness theorem} for first-order intuitionistic logic with respect to classical models in \bSet.
That is, anything we can derive using the rules of \cref{sec:fol} is necessarily \emph{true} as a statement about sets and their elements.
Namely, if we can prove $\Gamma\cb ()\types \ph$, then we must have a morphism, in the cartesian multiposet of subsets of the interpretation of $\Gamma$, from the empty list to $\ph$; which means that $\ph$ must be interpreted by the top element, i.e.\ $\ph$ is ``true'' about all elements of $\Gamma$.
In \cref{sec:subobjects} we will generalize this to hyperdoctrines constructed from more general categories.
...
% Since the diagonal has a retraction, the whole left adjoint can be recovered from its value at $\top$.
% Now the Hopf left adjoint property gives general transport, which by instantiation gives Paulin-Mohring.
% This (together with $\exis$) should allow us to construct left adjoints to pullback along arbitrary morphisms by Lawvere's formula, and also to prove BC for ``sliding and splitting'' and the ``Frobenius law''.
