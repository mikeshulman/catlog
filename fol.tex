...
Thus, in this section we begin with a version of first-order logic in which the cut rule \emph{for propositions} is primitive, allowing us to formulate all the new rules in what I find to be the most intuitive way.
...
Since we are only interested (at the moment) in whether or not an entailment is derivable (corresponding to semantics in posets), having a primitive cut rule is not really a problem.
...
\end{mathpar}
As usual, cut/substitution is admissible for \emph{this} theory.
...
We also mention at this point some non-primitive rules that we will eventually prove to be admissible.
The first is cut for propositions, which is just the cut rule from \cref{sec:monoidal-lattices} with an extra type context $\Gamma$:
\[ \inferrule{\Gamma\cb\Theta \types \ph \\ \Gamma\cb \Psi,\ph\types \psi}{\Gamma\cb \Psi,\Theta\types \psi}\]
The others are the substitution of \emph{terms into propositions} and entailments:
\begin{mathpar}
...
\end{mathpar}
The admissibility of cut, and of substitution into entailments, is not essential.
We could take these rules to be primitive without much loss: at the moment we are interested only in whether an entailment is derivable, not what its derivations are, so introducing new extra derivations that involve such primitive rules would not matter much.

It \emph{is} important, however, that substitution of terms into propositions be admissible, the same reason as admissibility of substitution into terms.
...
Admissibility of substitution into entailments (though not as important) is ensured similarly (and admissibility of cut is ensured as usual by a sequent calculus left/right rule structure).
...
\begin{mathpar}
  \inferrule{\Gamma\cb\Theta\types A}{\Gamma\cb\Theta\types A\join B}\;\joinR1
  \and
  \inferrule{\Gamma\cb\Theta\types B}{\Gamma\cb\Theta\types A\join B}\;\joinR2
  \and
  \inferrule{
...
Recall that in sequent calculus, each type operation has both one or more \emph{left} rules and one or more \emph{right} rules.
...
This suggests the following right rule:
\begin{mathpar}
  \inferrule{\Gamma,x:A\cb \Theta\types \ph}{\Gamma\cb\Theta\types \forall x:A.\ph}\;\forallR
\end{mathpar}
...
This suggests the following left rule:
\begin{mathpar}
  \inferrule{\Gamma\types M:A \\ \Gamma\cb \Theta, \ph[M/x] \types \psi}{\Gamma\cb \Theta, (\forall x:A.\ph) \types \psi}\;\forallL
