...
Of course, intuitions differ from person to person, and the reader is free to make different choices when writing their own book.
...
For these reasons, we begin with a version of first-order logic in which the cut rule \emph{for propositions} is primitive, allowing us to formulate all the new rules however we please.
Since we are only interested (at the moment) in \emph{whether or not} an entailment is derivable rather than what all its derivations are (corresponding to semantics in posets rather than more general categories), it doesn't matter to have extra rules around.
But after introducing the rules for quantifiers and equality this way, we present sequent calculus and natural deduction versions in \cref{sec:fol-seqcalc,sec:fol-natded}; the latter will be necessary when we generalize away from the posetal case in \cref{sec:indexed-moncat}.
...
\end{mathpar}
As usual, cut/substitution is admissible for \emph{this} theory (where we do care about distinguishing between different morphisms).
...
Since our logic is posetal in this section, we simplify our lives by asserting any desired structural rules for the propositions as primitive, the way we did in \cref{sec:monoidal-lattices}.
...
Also as in \cref{sec:logic}, we simplify our lives by always assuming exchange; and of course we also always have the identity rule:
\[ \inferrule*{ }{\Gamma\cb \ph\types\ph} \]
As metioned above, we will take the cut rule for propositions to be primitive; this is just the cut rule from \cref{sec:monoidal-lattices} with an extra type context $\Gamma$.
\[ \inferrule{\Gamma\cb\Theta \types \ph \\ \Gamma\cb \Psi,\ph\types \psi}{\Gamma\cb \Psi,\Theta\types \psi}\]

There are also two new structural rules arising from the dependency of propositions on types.
The first is the \emph{substitution of terms into propositions}:
\begin{mathpar}
  \inferrule{\Gamma\types M:A \\ \Gamma,x:A\types \ph\prop}{\Gamma \types \ph[M/x] \prop}\and
\end{mathpar}
It \emph{is} important that we make this rule admissible, for the same reason as the admissibility of substitution into terms.
We certainly want to be \emph{able} to make such substitutions, but if we asserted them as primitive then (to maintain the unique correspondence between names for propositions $\ph$ and the derivations of $\Gamma\types \ph\prop$) we would have to introduce ``$\ph[M/x]$'' as basic syntax, rather than an operation on syntax.
...
The second new structural rule is \emph{substitution of terms into entailments}:
\begin{mathpar}
  \inferrule{\Gamma\types M:A \\ \Gamma,x:A\cb\Theta\types \ph}{\Gamma\cb\Theta[M/x] \types \ph[M/x]}\and
\end{mathpar}
Since this is a rule for entailment, it is not as important for it to be admissible, for the same reason as cut for propositions.
Thus, in this section we will take it to be primitive as well, allowing us to further simplify some of the rules for quantifiers and equality.
But in the sequent calculus and natural deduction versions in \cref{sec:fol-seqcalc,sec:fol-natded}, we will make this rule admissible as well, by building substitutions into the entailment judgment just as we do for the proposition judgment.

We summarize the \emph{primitive} structural rules for our theory in \cref{fig:fol-primstruc}.
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \inferrule*{\Gamma\types M:A \\ \Gamma,x:A\cb\Theta\types \ph}{\Gamma\cb\Theta[M/x] \types \ph[M/x]}\and
  \end{mathpar}
  \caption{Primitive structural rules for first-order logic}
  \label{fig:fol-primstruc}
\end{figure}
...
% TODO: Sequent calculus reference?
...
\end{mathpar}
and likewise we have rules for $\bot,\meet,\top,\tensor,\one$, and $\hom$.\footnote{Since cut is primitive, we could simplify the rules $\joinI1$ and $\joinI2$ to $\Gamma\cb A\types A\join B$ and $\Gamma\cb B\types A\join B$, or write $\joinE$ as a sequent-calculus-style left rule; but we will stick with the more familiar natural deduction versions.}
...
% and we could also formulate the rules with an unchanging proposition context as in \cref{sec:heyting-algebras}.
% TODO: Remark on this when we do natural deduction.
...
\end{mathpar}
But there are many other ways to say the same thing, including a sequent-calculus-style left rule and a natural-deduction-style elimination rule.
\begin{mathpar}
...
\end{mathpar}
All of these rules are inter-derivable in the presence of primitive cut and substitution.
...
\begin{ex}\label{ex:eq-frob-from-hom}
  ...
  \begin{mathpar}
    \inferrule{\Gamma\types M:A \\ \Gamma\types N:A \\\\ \Gamma,x:A,y:A\types \ph\prop \\ \Gamma,x:A\cb\ec\types \ph[x/y]}{\Gamma\cb(M=_A N)\types \ph[M/x,N/y]}
  \end{mathpar}
\end{ex}
...
\begin{mathpar}
  ...
  x:A \cb \alpha(x) \types \exists y:B.(\beta(y)\meet\ph(x,y))\and
  x:A,y_1:B,y_2:B \cb \alpha(x), \beta(y_1), \beta(y_2), \ph(x,y_1), \ph(x,y_2) \types (y_1=y_2)
\end{mathpar}
