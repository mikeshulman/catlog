...
(However, there are still other technical advantages to admissibility, some of which we will see later.)
...
% Formally, we will want $\forallL$ in \cref{sec:fol-seqcalc}, $\forallE$ in \cref{sec:fol-natded}, and $\forallI^{-1}$ in \cref{sec:hyperdoctrines}.
...
% Technically, we will want $\existsL/\existsI$ in \cref{sec:fol-seqcalc}, $\existsE/\existsI$ in \cref{sec:fol-natded}, and $\existsL/\existsL^{-1}$ in \cref{sec:hyperdoctrines}.
...
\end{mathpar}
Secondly, to make substitution into entailments admissible, it would need substitutions for $M$ and $N$ built in:
...
\end{equation}
Thirdly, to make cut for propositions admissible, it will need some further cuts built in as well; see \cref{fig:fol-natded}.
...
\begin{enumerate}
\item The rules for terms and propositions summarized in \cref{fig:fol-common};
