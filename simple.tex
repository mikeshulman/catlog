...
As usual, we can generalize from multigraphs to a notion of \textbf{$(\times,+,\to)$-presentation}.

% TODO: another subsection
...
[TODO: Need $\catctx{-}$ in the cartesian case.  Say something about action of structural rules too.  This left adjoint reappears in the next subsection as \cref{thm:free-catprod-cartmulti}.]

The necessary definition of a \textbf{1-skeleton for a cartesian closed category with coproducts} is much simpler than in the non-cartesian monoidal case of \cref{sec:tensor-presentations}, since we can just use ordinary products of presheaves instead of convolution products.
Explicitly, it consists of the following (compare to the rules in \cref{fig:stlc}).
\begin{enumerate}
...
\item For each $A,B\in\cM_0$, morphisms $A\times B\to A$ and $A\times B\to B$ and a natural transformation
...
\end{enumerate}
We can then construct a tower of adjunctions as usual.
