...
\begin{ex}\label{ex:combinatory-logic}\ 
  ...
  \item Let \textbf{CL} denote the type theory obtained from simply typed $\lambda$-calculus by \emph{removing} the $\lambda$-abstraction rule:
    ...
    That is, given a derivation in \textbf{CL} of $\Gamma,x:A \types M:B$, we can construct a derivation in \textbf{CL} of $\Gamma\types [x]M:A\to B$ for some $[x]M$ (note that this is an \emph{operation} on terms of \textbf{CL}, like substitution).\label{item:cl2}
  \item Write down some $\equiv$-laws satisfied by the $I$, $K$, and $S$ you defined in~\ref{item:cl1}, and show that when they are used as generators for an $\equiv$ on \textbf{CL} it also presents a free closed cartesian multicategory.
